...
// Fungsi Helper Toast (Notif kecil di pojok)
...
async function selectCountry(cname, pricelist, countryNumberId) {
    ...
    container.innerHTML = '<div class="text-center p-5 opacity-50 text-[10px]">MEMUAT...</div>';
    ...
    try {
        ...
        if (result.success) {
            ...
            Swal.fire({
                ...
                text: 'Silakan tunggu kode OTP lo muncul.',
                confirmButtonText: 'Siap, Manzzy!'
            });
        } else {
            Swal.fire('Gagal!', result.msg || "Cek saldo lo.", 'error');
        }
    ...
    container.innerHTML = activeOrders.map(order => `
        <div class="galaxy-card p-4 rounded-2xl border-l-4 ${order.otp_code ? 'border-green-500' : 'border-purple-500'} mb-3">
            <div class="flex justify-between items-start mb-3">
                ...
                <button onclick="copyText('${order.phone_number}')" class="bg-white/5 p-2 rounded-lg text-[10px] hover:bg-purple-600 transition"><i class="fa-solid fa-copy"></i></button>
            </div>
            <div class="bg-white/5 rounded-xl p-3 text-center border ${order.otp_code ? 'border-green-500/50' : 'border-white/5'}">
                <span class="text-[8px] text-gray-500 block mb-1 uppercase">${order.otp_code ? 'OTP DITERIMA' : 'Status: ' + order.status}</span>
            ...
            <div class="mt-3 flex justify-between items-center text-[9px]">
                <div id="timer-${order.order_id}" class="text-gray-500 font-mono italic">${order.status === 'completed' ? 'Selesai' : '20:00'}</div>
                <div class="flex gap-2">
                    ${order.otp_code ? `
                        <button onclick="copyText('${order.otp_code}')" class="text-green-500 font-bold uppercase hover:underline">Salin OTP</button>
                    ` : `
            ...
            if (result.success && result.data.status === 'completed' && result.data.otp_code) {
                ...
                if (order && !order.otp_code) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'OTP MASUK!',
                        text: `Kode buat ${order.service}: ${order.otp_code}`,
                        confirmButtonText: 'Salin Kode'
                    }).then(() => copyText(order.otp_code));
                }
...
function startTimer(orderId, duration) {
    ...
    const interval = setInterval(() => {
        ...
        if (--timer < 0) { 
            clearInterval(interval); activeOrders = activeOrders.filter(o => o.order_id !== orderId); renderPendingOrders(); 
        }
    }, 1000);
}
...
async function cancelOrder(orderId) {
    const result = await Swal.fire({
        ...
        text: "Saldo lo bakal dikembaliin kok.",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Ya, Batalin!',
    ...
    if (result.isConfirmed) {
        ...
        } catch (e) { Swal.fire('Error', 'Gagal membatalkan', 'error'); }
    }
}

function copyText(txt) {
    navigator.clipboard.writeText(txt);
    Toast.fire({ icon: 'success', title: 'Teks Berhasil Disalin!' });
}